...
class StubStreamingManager:
    ...
    async def emit_event(self, request_id: str, event: Any) -> None:
        self.events.append({"type": str(event.event if hasattr(event, "event") else event), "data": event.data if hasattr(event, "data") else {}})
...
class StubClaimManager:
    ...
    def validate_claim(self, claim: Any) -> Any:
        return claim
