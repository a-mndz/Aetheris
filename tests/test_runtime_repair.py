...
class TestPhase3ResourceOwnership:
    @pytest.mark.asyncio
    async def test_rejected_acquisition_never_executes_or_releases(self) -> None:
        ...
        class ResourceManager:
            ...
            @staticmethod
            async def acquire_resources(**kwargs):
                return False
        ...
        class Gateway:
            ...
            async def execute_with_fallback(**kwargs):
                raise AssertionError("rejected request reached gateway")

        resources = ResourceManager()
        runtime = RuntimeEngine(resource_manager=resources)

        with pytest.raises(RuntimeError, match="limit exceeded"):
            await runtime.execute_with_contracts(
                prompt="hello",
        ...
        assert resources.releases == 0

    @pytest.mark.asyncio
    async def test_success_releases_exactly_owned_permit(self) -> None:
        from core.passport import ExecutionPassport
        from core.runtime import RuntimeEngine
        ...
        result = await runtime.execute_with_contracts(
            prompt="hello",
            system_prompt="system",
            role="logician",
            passport=ExecutionPassport(),
