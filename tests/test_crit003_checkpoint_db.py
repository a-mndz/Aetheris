...
def _install_fake_db(monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    async def retrieve_db(self: CheckpointManager, checkpoint_id: str) -> Checkpoint | None:
        async with self._require_db_factory()() as session:
...
def test_database_backend_save_swallows_missing_factory():
    """Public save() must not raise when factory missing — errors are logged."""
    mgr = CheckpointManager(storage_backend="database", db_session_factory=None)
    ...
    cp_id = asyncio.get_event_loop().run_until_complete(_run())
    assert cp_id  # default returned via with_timeout is the candidate checkpoint id
