...
function HomeHero({ agentState, toggleAgent, onQuickPrompt, onFocusInput, onOpenSettings, onOpenTelemetry }) {
                ...
                <>
                  ...
                  <div className="agent-matrix">
                    {AGENTS.map((agent) => (
                      <button
                        key={agent.id}
                        className={`agent-tile${agentState[agent.id] ? " on" : ""}`}
                        onClick={() => toggleAgent(agent.id)}
                        aria-pressed={agentState[agent.id]}
